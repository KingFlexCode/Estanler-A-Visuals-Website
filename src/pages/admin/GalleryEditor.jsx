...
function getUniqueSlug(baseSlug, existingItems = []) {
  const cleanBase = baseSlug || "photo-set";
  const existingSlugs = new Set(existingItems.map((item) => item.slug).filter(Boolean));
  if (!existingSlugs.has(cleanBase)) return cleanBase;
  let counter = 2;
  while (existingSlugs.has(`${cleanBase}-${counter}`)) counter += 1;
...
function GalleryPreview({ gallery, sections, photos, coverPhoto, previewMode }) {
  ...
  const titleBlock = (align = "center", color = "#fff", options = {}) => <div style={{ textAlign: align, color, maxWidth: options.maxWidth || 760, textShadow: color === "#fff" ? "0 8px 24px rgba(0,0,0,0.35)" : "none", overflow: "hidden" }}><div style={{ color: themeColor, fontFamily: "'Inter', sans-serif", fontSize: options.dateSize || 11, fontWeight: 800, letterSpacing: "0.16em", marginBottom: "0.75rem", textTransform: "uppercase" }}>{formatDate(gallery.event_date)}</div><h1 style={{ fontFamily: typeTheme.family, fontSize: options.titleSize || (isMobile ? "clamp(1.45rem, 7vw, 2rem)" : "clamp(2rem, 4.4vw, 3.1rem)"), fontWeight: typeTheme.weight, letterSpacing: typeTheme.spacing, lineHeight: 1.02, margin: 0, maxWidth: "100%", overflowWrap: "anywhere", textTransform: typeTheme.transform }}>{gallery.title || "Untitled Gallery"}</h1>{gallery.client_name && <p style={{ fontFamily: "'Inter', sans-serif", fontSize: options.clientSize || 13, letterSpacing: "0.08em", margin: "1rem 0 0", overflowWrap: "anywhere", textTransform: "uppercase" }}>{gallery.client_name}</p>}</div>;

  const renderHero = () => {
    const compactTitle = { titleSize: isMobile ? "1.55rem" : "2.15rem", maxWidth: 390, dateSize: 10, clientSize: 11 };
    if (coverStyle === "novel") return <section style={{ minHeight: isMobile ? 330 : 420, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "42% 58%", background: "#fff", border: "18px solid #fff", boxSizing: "border-box" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", overflow: "hidden" }}>{titleBlock("left", "#111", compactTitle)}</div><div style={{ ...fullColorImageStyle, minHeight: isMobile ? 260 : 384 }} /></section>;
    if (coverStyle === "journal") return <section style={{ minHeight: isMobile ? 330 : 420, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "62% 38%", background: "#fff" }}><div style={{ ...fullColorImageStyle, minHeight: isMobile ? 280 : 420 }} /><div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", overflow: "hidden" }}>{titleBlock("left", "#111", compactTitle)}</div></section>;
    if (coverStyle === "minimal") return <section style={{ minHeight: isMobile ? 330 : 420, background: "#fff", color: "#111", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", boxSizing: "border-box" }}><div style={{ textAlign: "center", width: "100%", maxWidth: 520, overflow: "hidden" }}><div style={{ width: isMobile ? 160 : 220, aspectRatio: "1 / 1", margin: "0 auto 1.5rem", ...fullColorImageStyle }} />{titleBlock("center", "#111", { titleSize: isMobile ? "1.65rem" : "2.25rem", maxWidth: 520 })}</div></section>;
    if (coverStyle === "split") return <section style={{ minHeight: isMobile ? 330 : 420, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", background: "#101010" }}><div style={{ ...fullColorImageStyle, minHeight: isMobile ? 250 : 420 }} /><div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", overflow: "hidden" }}>{titleBlock("left", "#fff", compactTitle)}</div></section>;
    if (coverStyle === "vintage") return <section style={{ minHeight: isMobile ? 330 : 420, background: "#262626", padding: isMobile ? "1rem" : "1.45rem", boxSizing: "border-box", display: "grid", gridTemplateRows: "minmax(220px, 1fr) auto", gap: 0 }}><div style={{ ...fullColorImageStyle, minHeight: isMobile ? 260 : 315 }} /><div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr auto 1fr", alignItems: "center", gap: "1rem", padding: isMobile ? "1.25rem 0.5rem 0.25rem" : "1.55rem 0.6rem 0.35rem", color: "#fff", overflow: "hidden" }}><div style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, letterSpacing: "0.16em", textTransform: "uppercase", opacity: 0.85 }}>{gallery.client_name || "Estanler Aleman Photography"}</div><div style={{ textAlign: "center", overflow: "hidden" }}>{titleBlock("center", "#fff", { titleSize: isMobile ? "1.55rem" : "2rem", maxWidth: 520, dateSize: 10, clientSize: 11 })}</div><div style={{ textAlign: isMobile ? "left" : "right" }}><button type="button" style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 800, letterSpacing: "0.14em", padding: "0.8rem 1.2rem", textTransform: "uppercase" }}>View Gallery</button></div></div></section>;
    if (coverStyle === "divider") return <section style={{ minHeight: isMobile ? 330 : 420, ...fullColorImageStyle, position: "relative", overflow: "hidden", display: "flex", alignItems: "stretch", justifyContent: "flex-start" }}><div style={{ width: isMobile ? "100%" : "42%", minHeight: isMobile ? 330 : 420, background: "rgba(0,0,0,0.54)", backdropFilter: "blur(1px)", display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "2rem" : "2.65rem", boxSizing: "border-box" }}><div style={{ width: "100%", maxWidth: 430, overflow: "hidden" }}>{titleBlock("center", "#fff", { titleSize: isMobile ? "1.65rem" : "2.2rem", maxWidth: 430, dateSize: 10, clientSize: 12 })}<button type="button" style={{ display: "block", margin: "2rem auto 0", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.65)", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 800, letterSpacing: "0.14em", padding: "0.8rem 1.35rem", textTransform: "uppercase" }}>View Gallery</button></div></div></section>;
...
export default function GalleryEditor() {
  ...
  useEffect(() => {
    ...
    const uploadComplete = uploadQueue.every((item) => isFinalUploadStatus(item.status));
    const hasFailed = uploadQueue.some((item) => item.status === "failed");
    if (!uploadComplete || hasFailed) return undefined;
    const timer = window.setTimeout(() => setUploadQueue([]), 8000);
  ...
  function closeDeleteModal() { if (deleting) return; setDeleteTarget(null); setDeleteConfirmText(""); setDeleteModalError(""); }
  ...
  async function deleteSection(section) { if (!section?.id) return false; const sectionPhotos = photos.filter((photo) => photo.section_id === section.id); const sectionPhotoIds = sectionPhotos.map((photo) => photo.id); if (sectionPhotoIds.includes(gallery.cover_image_id)) { const { data, error: coverError } = await supabase.from("client_galleries").update({ cover_image_id: null }).eq("id", gallery.id).select("*").single(); if (coverError) { setNotice(""); setError(coverError.message); return false; } setGallery(data); } if (sectionPhotoIds.length > 0) { const { error: photoDeleteError } = await supabase.from("client_gallery_images").delete().in("id", sectionPhotoIds); if (photoDeleteError) { setNotice(""); setError(photoDeleteError.message); return false; } } const { error: sectionDeleteError } = await supabase.from("client_gallery_sections").delete().eq("id", section.id); if (sectionDeleteError) { setNotice(""); setError(sectionDeleteError.message); return false; } const remainingSections = sections.filter((item) => item.id !== section.id); setSections(remainingSections); setPhotos((current) => current.filter((photo) => photo.section_id !== section.id)); setSelectedPhotoIds([]); setSelectionAnchorId(null); setTargetSection(remainingSections[0]?.id || ""); flash("Photo set deleted."); return true; }
  ...
  function renderDeleteModal() {
    ...
    const name = isGallery ? gallery?.title || "Untitled Gallery" : deleteTarget.record?.title || "Untitled Set";
    const photoCount = isGallery ? photos.length : photos.filter((photo) => photo.section_id === deleteTarget.record?.id).length;
    return <div style={{ position: "fixed", inset: 0, zIndex: 320, background: "rgba(0,0,0,0.62)", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}><div style={{ width: "min(540px, 94vw)", background: "#101820", border: `1px solid rgba(255,139,139,0.42)`, boxShadow: "0 30px 100px rgba(0,0,0,0.55)", color: COLORS.white }}><div style={{ padding: "1.5rem 1.6rem", borderBottom: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", gap: "1rem" }}><div><div style={{ color: "#ff8b8b", fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 900, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 6 }}>Danger Zone</div><h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.55rem", margin: 0 }}>Delete {isGallery ? "Gallery" : "Photo Set"}</h2></div><button type="button" onClick={closeDeleteModal} disabled={deleting} style={{ marginLeft: "auto", background: "transparent", border: "none", color: COLORS.muted, cursor: deleting ? "not-allowed" : "pointer", fontSize: 26 }}>×</button></div><div style={{ padding: "1.5rem 1.6rem" }}><p style={{ color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 13, lineHeight: 1.7, margin: "0 0 1rem" }}>You are about to delete <strong style={{ color: COLORS.white }}>"{name}"</strong>. {isGallery ? "This removes the gallery, photo sets, and gallery photo records." : `This removes this photo set and ${photoCount} photo${photoCount === 1 ? "" : "s"} inside it.`}</p><label><FieldLabel>Type DELETE to confirm</FieldLabel><input value={deleteConfirmText} onChange={(event) => { setDeleteConfirmText(event.target.value); setDeleteModalError(""); }} placeholder="DELETE" autoFocus style={{ ...inputStyle, borderColor: deleteModalError ? "rgba(255,139,139,0.75)" : COLORS.border }} /></label>{deleteModalError && <div style={{ color: "#ff8b8b", fontFamily: "'Inter', sans-serif", fontSize: 12, marginTop: 10 }}>{deleteModalError}</div>}<div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: "1.4rem" }}><button type="button" onClick={closeDeleteModal} disabled={deleting} style={buttonStyle}>Cancel</button><button type="button" onClick={confirmDeleteModal} disabled={deleting} style={{ ...buttonStyle, background: "#ff8b8b", borderColor: "#ff8b8b", color: "#101820" }}>{deleting ? "Deleting..." : `Delete ${isGallery ? "Gallery" : "Set"}`}</button></div></div></div></div>;
  }
