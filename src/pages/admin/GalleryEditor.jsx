...
function StatusPill({ status, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      style={{ ...buttonStyle, borderRadius: 999, borderColor: color, color, padding: "5px 10px" }}
...
function SettingCard({ active, title, description, onClick }) {
  return (
    ...
    >
      <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase" }}>
        {title}
      </div>
      {description && <div style={{ color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 11, lineHeight: 1.5, marginTop: 5 }}>{description}</div>}
    </button>
  );
}
...
function UploadProgressPanel({ queue, uploading, elapsedSeconds }) {
  ...
  return (
    <div style={{ border: `1px solid ${COLORS.border}`, background: "rgba(255,255,255,0.025)", padding: "0.85rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem", marginBottom: 8 }}>
        <div>
          <FieldLabel>Upload Progress</FieldLabel>
          <div style={{ color: COLORS.white, fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 800 }}>{uploading ? "Uploading gallery photos" : "Upload complete"}</div>
          {activeItem && <div style={{ color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 11, marginTop: 4 }}>Current: {activeItem.name}</div>}
        </div>
        <div style={{ color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 11, textAlign: "right" }}>
          ...
          {remaining !== null && <div>~{formatDuration(remaining)} left</div>}
        </div>
      </div>
      <div style={{ height: 8, border: `1px solid ${COLORS.border}`, background: "rgba(255,255,255,0.04)", overflow: "hidden", marginBottom: 8 }}>
        <div style={{ width: `${Math.max(0, Math.min(100, totalProgress))}%`, height: "100%", background: COLORS.gold, transition: "width 0.2s ease" }} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 11, marginBottom: 10 }}>
...
function GalleryPreview({ gallery, sections, photos, coverPhoto, previewMode }) {
  ...
  const renderSectionPhotos = (sectionPhotos) => {
    if (gridStyle === "square") {
      return (
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${isMobile ? 2 : 4}, minmax(0, 1fr))`, gap: 8, overflow: "hidden" }}>
          {sectionPhotos.map((photo) => (
            <div key={photo.id} style={{ aspectRatio: "1 / 1", background: "#f2f2f2", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <img src={getPhotoPreviewUrl(photo)} alt={photo.alt_text || photo.title || "Gallery photo"} style={{ width: "100%", height: "100%", objectFit: "contain", display: "block" }} />
            </div>
          ))}
        </div>
      );
    ...
    return (
      <div style={{ columnCount: isMobile ? 2 : 3, columnGap: gridStyle === "editorial" ? 14 : 8, width: "100%", overflow: "hidden" }}>
        {sectionPhotos.map((photo) => (
          <img
            key={photo.id}
            src={getPhotoPreviewUrl(photo)}
            alt={photo.alt_text || photo.title || "Gallery photo"}
            style={{ width: "100%", height: "auto", display: "block", breakInside: "avoid", marginBottom: gridStyle === "editorial" ? 14 : 8 }}
          />
        ))}
      </div>
    );
  ...
  return (
    <div style={{ width: "100%", maxWidth: isMobile ? 390 : 980, margin: "0 auto", background: "#fff", color: "#111", boxShadow: "0 28px 90px rgba(0,0,0,0.35)", overflow: "hidden", transition: "max-width 0.25s ease" }}>
      <section
        style={{
          minHeight: isMobile ? 330 : 420,
          background: coverUrl ? `linear-gradient(rgba(0,0,0,0.18), rgba(0,0,0,0.48)), url(${coverUrl}) ${objectPosition} / cover` : "linear-gradient(135deg, #1c1c1c, #3b3b3b)",
          display: "flex",
      ...
      >
        {coverStyle === "stripe" && <span style={{ position: "absolute", left: "12%", right: "12%", top: "50%", height: 1, background: "rgba(255,255,255,0.75)" }} />}
        <div style={{ textAlign: titleAlign, color: "#fff", maxWidth: 760 }}>
          <div style={{ color: themeColor, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 800, letterSpacing: "0.16em", marginBottom: "0.75rem", textTransform: "uppercase" }}>{formatDate(gallery.event_date)}</div>
          <h1 style={{ fontFamily: headingFont, fontSize: isMobile ? "2rem" : "3.1rem", fontWeight: typography === "modern" ? 700 : 600, letterSpacing: typography === "editorial" ? "0.18em" : "0.02em", lineHeight: 1, margin: 0, textTransform: typography === "editorial" ? "uppercase" : "none" }}>{gallery.title || "Untitled Gallery"}</h1>
          {gallery.client_name && <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, letterSpacing: "0.08em", margin: "1rem 0 0", textTransform: "uppercase" }}>{gallery.client_name}</p>}
        </div>
      </section>
      <section style={{ padding: isMobile ? "1.25rem" : "2rem", overflow: "hidden" }}>
        {visibleSections.length === 0 && <div style={{ color: "#777", fontFamily: "'Inter', sans-serif", textAlign: "center" }}>No visible photo sets yet.</div>}
        {visibleSections.map((section) => {
          const sectionPhotos = sortByOrder(photos).filter((photo) => photo.section_id === section.id);
          return (
            <div key={section.id} style={{ marginBottom: isMobile ? "1.75rem" : "2.75rem", overflow: "hidden" }}>
              <h2 style={{ color: "#111", fontFamily: headingFont, fontSize: isMobile ? "1.15rem" : "1.45rem", fontWeight: 600, margin: "0 0 1rem" }}>{section.title}</h2>
              {renderSectionPhotos(sectionPhotos)}
            </div>
          );
        })}
      </section>
...
export default function GalleryEditor() {
  ...
  async function moveSelectedByStep(direction) {
    ...
    const selectedIndexes = activeSectionPhotos
      .map((photo, index) => selectedSet.has(photo.id) ? index : -1)
    ...
    const insertIndex = direction === "up"
      ...
      : Math.min(remaining.length, firstSelectedIndex + 1);
    const nextOrder = [...remaining.slice(0, insertIndex), ...selected, ...remaining.slice(insertIndex)];

    if (await persistPhotoOrder(nextOrder)) {
      flash(direction === "up" ? "Moved selected photos up." : "Moved selected photos down.");
    }
    setActionMenuOpen(false);
  }
  ...
  async function removePhoto(photoId) {
    ...
    if (gallery.cover_image_id === photoId) setGalleryField("cover_image_id", null);
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        <div>
          ...
          <input ref={fileInputRef} type="file" multiple accept="image/*" onChange={(event) => uploadSelectedFiles(event.target.files)} style={{ display: "none" }} />
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading || !activeSection} style={{ ...primaryButtonStyle, width: "100%", opacity: uploading || !activeSection ? 0.55 : 1, cursor: uploading || !activeSection ? "not-allowed" : "pointer" }}>{uploading ? "Uploading..." : "+ Upload Photos"}</button>
          <p style={{ color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 12, lineHeight: 1.6, margin: "0.65rem 0 0" }}>Uploads go into the selected photo set and belong only to this client gallery.</p>
        </div>
        <UploadProgressPanel queue={uploadQueue} uploading={uploading} elapsedSeconds={elapsedSeconds} />
        <label><FieldLabel>Upload Into Photo Set</FieldLabel><select value={targetSection} onChange={(event) => setTargetSection(event.target.value)} style={inputStyle}>{sections.map((section) => <option key={section.id} value={section.id}>{section.title}</option>)}</select></label>
        <div><FieldLabel>Create Photo Set</FieldLabel><div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8 }}><input value={newSection} onChange={(event) => setNewSection(event.target.value)} placeholder="Reception, Ceremony, Portraits..." style={inputStyle} /><button type="button" onClick={addSection} style={buttonStyle}>Add</button></div></div>
        <div><FieldLabel>Photo Sets</FieldLabel><div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{sections.length === 0 && <EmptyState>Create a photo set before uploading images.</EmptyState>}{sortByOrder(sections).map((section) => { const count = photos.filter((photo) => photo.section_id === section.id).length; const active = activeSection?.id === section.id; return <button type="button" key={section.id} onClick={() => { setTargetSection(section.id); setSelectedPhotoIds([]); setSelectionAnchorId(null); setContextMenu(null); }} style={{ ...buttonStyle, background: active ? "rgba(255,255,255,0.075)" : "rgba(255,255,255,0.025)", borderColor: active ? COLORS.gold : COLORS.border, color: active ? COLORS.white : COLORS.muted, display: "flex", justifyContent: "space-between", textAlign: "left" }}><span>{section.title || "Untitled Set"}</span><span>{count}</span></button>; })}</div></div>
        {activeSection && <section style={{ border: `1px solid ${COLORS.border}`, padding: "0.85rem" }}><FieldLabel>Selected Set</FieldLabel><input value={activeSection.title || ""} onChange={(event) => setSections((current) => current.map((item) => item.id === activeSection.id ? { ...item, title: event.target.value } : item))} style={{ ...inputStyle, marginBottom: 8 }} /><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><button type="button" onClick={() => saveSection(activeSection)} style={buttonStyle}>Save Set</button><button type="button" onClick={() => saveSection(activeSection, { is_visible: activeSection.is_visible === false })} style={buttonStyle}>{activeSection.is_visible === false ? "Show" : "Hide"}</button></div></section>}
      </div>
    );
  }
  ...
  function renderDesignPanel() {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem", paddingBottom: "2rem" }}>
        <div><FieldLabel>Device Preview</FieldLabel><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><button type="button" onClick={() => setPreviewMode("desktop")} style={{ ...buttonStyle, color: previewMode === "desktop" ? COLORS.gold : COLORS.white }}>Desktop</button><button type="button" onClick={() => setPreviewMode("mobile")} style={{ ...buttonStyle, color: previewMode === "mobile" ? COLORS.gold : COLORS.white }}>Mobile</button></div></div>
        <div><FieldLabel>Cover Style</FieldLabel><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>{COVER_STYLES.map((style) => <SettingCard key={style.id} active={(gallery.cover_style || "center") === style.id} title={style.label} description={style.description} onClick={() => setGalleryField("cover_style", style.id)} />)}</div></div>
        <label><FieldLabel>Gallery Color</FieldLabel><input type="color" value={gallery.theme_color || "#c8a96a"} onChange={(event) => setGalleryField("theme_color", event.target.value)} style={{ ...inputStyle, minHeight: 44, padding: 6 }} /></label>
        <div><FieldLabel>Grid Style</FieldLabel><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>{GRID_STYLES.map((style) => <button key={style} type="button" onClick={() => setGalleryField("grid_style", style)} style={{ ...buttonStyle, color: (gallery.grid_style || "masonry") === style ? COLORS.gold : COLORS.white }}>{style}</button>)}</div></div>
        <div><FieldLabel>Typography</FieldLabel><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>{TYPOGRAPHY_STYLES.map((style) => <button key={style} type="button" onClick={() => setGalleryField("typography_style", style)} style={{ ...buttonStyle, color: (gallery.typography_style || "classic") === style ? COLORS.gold : COLORS.white }}>{style}</button>)}</div></div>
        <div><FieldLabel>Cover Focal Point</FieldLabel><label style={{ display: "block", marginBottom: 10 }}><span style={{ color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 11 }}>Horizontal</span><input type="range" min="0" max="100" value={gallery.cover_focal_x ?? 50} onChange={(event) => setGalleryField("cover_focal_x", Number(event.target.value))} style={{ width: "100%" }} /></label><label style={{ display: "block" }}><span style={{ color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 11 }}>Vertical</span><input type="range" min="0" max="100" value={gallery.cover_focal_y ?? 50} onChange={(event) => setGalleryField("cover_focal_y", Number(event.target.value))} style={{ width: "100%" }} /></label></div>
      </div>
    );
  }
  ...
  function renderPhotosWorkspace() {
    ...
    return (
      <main onClick={() => { setContextMenu(null); setActionMenuOpen(false); }} style={{ background: "#f4f4f4", color: "#111", height: "calc(100vh - 124px)", overflowY: "auto", overflowX: "hidden", padding: "2rem", position: "relative", boxSizing: "border-box" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", marginBottom: "1.5rem" }}><div><h2 style={{ fontFamily: "'Inter', sans-serif", fontSize: "1.7rem", margin: 0 }}>{activeSection.title || "Untitled Set"}</h2><p style={{ color: "#777", fontFamily: "'Inter', sans-serif", fontSize: 13, margin: "0.4rem 0 0" }}>{activeSectionPhotos.length} photo{activeSectionPhotos.length === 1 ? "" : "s"} · Click to select · Shift-click selects a range · Drag to reorder · Right-click for actions</p></div><div style={{ display: "flex", gap: 8 }}><button type="button" onClick={() => { setSelectedPhotoIds(activeSectionPhotos.map((photo) => photo.id)); setSelectionAnchorId(activeSectionPhotos[0]?.id || null); }} style={{ ...buttonStyle, color: "#111", borderColor: "#ddd" }}>Select All</button><button type="button" onClick={() => { setSelectedPhotoIds([]); setSelectionAnchorId(null); }} style={{ ...buttonStyle, color: "#111", borderColor: "#ddd" }}>Clear</button></div></div>
        {activeSectionPhotos.length === 0 && <div style={{ border: "1px dashed #ccc", color: "#777", fontFamily: "'Inter', sans-serif", padding: "4rem 2rem", textAlign: "center" }}>No photos in this set yet. Use the Photos panel to upload images.</div>}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: "1.25rem" }}>{activeSectionPhotos.map((photo) => { const selected = selectedPhotoIds.includes(photo.id); const isCover = gallery.cover_image_id === photo.id; return <article key={photo.id} draggable onDragStart={() => setDraggedPhotoId(photo.id)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); reorderDraggedPhoto(photo.id); }} onClick={(event) => togglePhotoSelection(photo.id, event)} onContextMenu={(event) => { event.preventDefault(); setContextMenu({ photoId: photo.id, x: event.clientX, y: event.clientY }); if (!selectedPhotoIds.includes(photo.id)) { setSelectedPhotoIds([photo.id]); setSelectionAnchorId(photo.id); } }} style={{ background: "#fff", border: `2px solid ${selected ? "#00b894" : isCover ? COLORS.gold : "transparent"}`, boxShadow: selected ? "0 0 0 3px rgba(0,184,148,0.16)" : "none", cursor: "grab", padding: "0.7rem", position: "relative", userSelect: "none" }}>{isCover && <span style={{ position: "absolute", left: 10, top: 10, background: COLORS.gold, color: COLORS.bg, fontFamily: "'Inter', sans-serif", fontSize: 9, fontWeight: 900, letterSpacing: "0.1em", padding: "4px 6px", textTransform: "uppercase", zIndex: 2 }}>Cover</span>}<div style={{ aspectRatio: "4 / 3", background: "#f2f2f2", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}><img src={getPhotoPreviewUrl(photo)} alt={photo.alt_text || photo.title || "Gallery photo"} style={{ width: "100%", height: "100%", objectFit: "contain", display: "block" }} /></div><div style={{ color: "#555", fontFamily: "'Inter', sans-serif", fontSize: 11, marginTop: 8, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{photo.file_name || photo.title || photo.id}</div></article>; })}</div>
        {contextMenu && contextPhoto && <div onClick={(event) => event.stopPropagation()} style={{ position: "fixed", left: contextMenu.x, top: contextMenu.y, zIndex: 150, minWidth: 220, background: "#fff", border: "1px solid #ddd", boxShadow: "0 18px 55px rgba(0,0,0,0.18)", padding: "0.5rem" }}>{[["Open", () => openPhoto(contextPhoto)], ["Set as cover", () => setCoverImage(contextPhoto.id)], ["Copy filename", () => copyFilenames([contextPhoto])], ["Move up", () => moveSelectedByStep("up")], ["Move down", () => moveSelectedByStep("down")], ["Watermark later", () => showLaterNotice("Watermark")], ["Create mobile app later", () => showLaterNotice("Mobile app")], ["Delete", () => removePhoto(contextPhoto.id)]].map(([label, action]) => <button type="button" key={label} onClick={action} style={{ display: "block", width: "100%", background: "transparent", border: "none", color: label === "Delete" ? "#c0392b" : "#222", cursor: "pointer", fontFamily: "'Inter', sans-serif", fontSize: 13, padding: "0.7rem 0.85rem", textAlign: "left" }}>{label}</button>)}</div>}
        {selectedPhotoIds.length > 0 && <div onClick={(event) => event.stopPropagation()} style={{ position: "fixed", left: "50%", bottom: 24, transform: "translateX(-50%)", zIndex: 120, width: "min(92vw, 760px)", background: "#171717", color: "#fff", boxShadow: "0 20px 60px rgba(0,0,0,0.35)", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "0.8rem 1rem", boxSizing: "border-box" }}><button type="button" onClick={() => { setSelectedPhotoIds([]); setSelectionAnchorId(null); }} style={{ ...buttonStyle, border: "none", padding: "6px 8px" }}>✕</button><strong style={{ fontFamily: "'Inter', sans-serif", fontSize: 14, marginRight: "auto" }}>{selectedPhotoIds.length} selected</strong><button type="button" onClick={() => showLaterNotice("Favorites")} style={buttonStyle}>☆ Favorite</button><button type="button" onClick={() => showLaterNotice("Quick share")} style={buttonStyle}>🔗 Share</button><button type="button" onClick={deleteSelectedPhotos} style={{ ...buttonStyle, color: "#ff8b8b" }}>Delete</button><div style={{ position: "relative", marginLeft: "auto" }}><button type="button" onClick={() => setActionMenuOpen((open) => !open)} style={{ ...buttonStyle, minWidth: 44 }}>...</button>{actionMenuOpen && <div style={{ position: "absolute", right: 0, bottom: "calc(100% + 10px)", minWidth: 240, background: "#fff", color: "#111", border: "1px solid #ddd", boxShadow: "0 18px 55px rgba(0,0,0,0.2)", padding: "0.5rem" }}><button type="button" onClick={() => selectedPhotoIds[0] && setCoverImage(selectedPhotoIds[0])} style={{ display: "block", width: "100%", background: "transparent", border: "none", color: "#111", cursor: "pointer", padding: "0.65rem", textAlign: "left" }}>Set first selected as cover</button><button type="button" onClick={() => moveSelectedByStep("up")} style={{ display: "block", width: "100%", background: "transparent", border: "none", color: "#111", cursor: "pointer", padding: "0.65rem", textAlign: "left" }}>Move up</button><button type="button" onClick={() => moveSelectedByStep("down")} style={{ display: "block", width: "100%", background: "transparent", border: "none", color: "#111", cursor: "pointer", padding: "0.65rem", textAlign: "left" }}>Move down</button><div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 6, padding: "0.45rem" }}><select value={moveTargetSectionId} onChange={(event) => setMoveTargetSectionId(event.target.value)} style={{ color: "#111", border: "1px solid #ddd", padding: "8px" }}>{sections.map((section) => <option key={section.id} value={section.id}>{section.title}</option>)}</select><button type="button" onClick={() => moveSelectedToSection(moveTargetSectionId)} style={{ color: "#111", border: "1px solid #ddd", background: "#fff", cursor: "pointer", padding: "8px" }}>Move</button></div><button type="button" onClick={() => copyFilenames()} style={{ display: "block", width: "100%", background: "transparent", border: "none", color: "#111", cursor: "pointer", padding: "0.65rem", textAlign: "left" }}>Copy Names</button></div>}</div></div>}
      </main>
    );
  }
  ...
  return (
    <div style={{ ...pageStyle, height: "100vh", overflow: "hidden" }}>
      <AdminNav onSignOut={handleSignOut} />
      <header style={{ height: 68, borderBottom: `1px solid ${COLORS.border}`, background: COLORS.surfaceDark || "#060606", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem", padding: "0 1.5rem", position: "sticky", top: 56, zIndex: 40 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "1rem", minWidth: 0 }}><button type="button" onClick={() => navigate("/admin/galleries")} style={buttonStyle}>← Galleries</button><div style={{ minWidth: 0 }}><h1 style={{ color: COLORS.white, fontFamily: "'Playfair Display', serif", fontSize: "1.15rem", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{gallery.title || "Untitled Gallery"}</h1><div style={{ color: COLORS.muted, fontFamily: "'Inter', sans-serif", fontSize: 12 }}>{formatDate(gallery.event_date)} · {photos.length} photo{photos.length === 1 ? "" : "s"}</div></div><StatusPill status={gallery.status} onClick={toggleGalleryVisibility} /></div>
        <div style={{ display: "flex", gap: "0.65rem", alignItems: "center" }}><button type="button" onClick={openPreview} style={buttonStyle}>Preview</button><button type="button" disabled style={{ ...buttonStyle, opacity: 0.45, cursor: "not-allowed" }}>Share Later</button><button type="button" onClick={saveGallery} disabled={saving} style={primaryButtonStyle}>{saving ? "Saving..." : "Save Gallery"}</button><button type="button" onClick={deleteGallery} style={{ ...buttonStyle, color: "#ff8b8b", borderColor: "rgba(255,139,139,0.45)" }}>Delete Gallery</button></div>
      </header>
      <div style={{ display: "grid", gridTemplateColumns: "360px minmax(0, 1fr)", height: "calc(100vh - 124px)", overflow: "hidden" }}>
        <aside style={{ borderRight: `1px solid ${COLORS.border}`, background: COLORS.surfaceDark || "#060606", height: "calc(100vh - 124px)", overflow: "hidden", display: "flex", flexDirection: "column" }} onWheel={(event) => event.stopPropagation()}>
          <button type="button" aria-label="Open cover design controls" onClick={() => setActiveTab("design")} style={{ aspectRatio: "16 / 9", width: "100%", backgroundColor: "rgba(255,255,255,0.035)", backgroundImage: getCoverUrl(coverPhoto) ? `url(${getCoverUrl(coverPhoto)})` : "linear-gradient(135deg, rgba(255,255,255,0.08), rgba(255,255,255,0.02))", backgroundPosition: `${gallery.cover_focal_x ?? 50}% ${gallery.cover_focal_y ?? 50}%`, backgroundSize: getCoverUrl(coverPhoto) ? "contain" : "cover", backgroundRepeat: "no-repeat", border: "none", borderBottom: `1px solid ${COLORS.border}`, cursor: "pointer", flexShrink: 0 }} />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", borderBottom: `1px solid ${COLORS.border}`, flexShrink: 0 }}>{SIDEBAR_TABS.map((tab) => <button key={tab.id} type="button" onClick={() => { setActiveTab(tab.id); setContextMenu(null); setActionMenuOpen(false); }} style={{ background: activeTab === tab.id ? "rgba(255,255,255,0.07)" : "transparent", border: "none", borderRight: `1px solid ${COLORS.border}`, color: activeTab === tab.id ? COLORS.gold : COLORS.muted, cursor: "pointer", fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 800, letterSpacing: "0.08em", padding: "0.75rem 0.35rem", textTransform: "uppercase" }} title={tab.label}><div style={{ fontSize: 16, marginBottom: 4 }}>{tab.icon}</div>{tab.label}</button>)}</div>
          <div style={{ flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden", padding: "1rem" }} onWheel={(event) => event.stopPropagation()}>
            {error && <div style={{ border: "1px solid rgba(224,92,92,0.35)", color: "#ff8b8b", fontFamily: "'Inter', sans-serif", fontSize: 12, lineHeight: 1.5, marginBottom: "1rem", padding: "0.75rem" }}>{error}</div>}
            {notice && <div style={{ border: "1px solid rgba(74,222,128,0.28)", color: "#9af0b8", fontFamily: "'Inter', sans-serif", fontSize: 12, lineHeight: 1.5, marginBottom: "1rem", padding: "0.75rem" }}>{notice}</div>}
            {renderActivePanel()}
          </div>
        </aside>
        {activeTab === "photos" ? renderPhotosWorkspace() : <main style={{ background: "#f4f4f4", height: "calc(100vh - 124px)", overflowY: "auto", overflowX: "hidden", padding: "3rem 2rem", boxSizing: "border-box" }}><GalleryPreview gallery={gallery} sections={sections} photos={photos} coverPhoto={coverPhoto} previewMode={previewMode} /></main>}
      </div>
    </div>
  );
}
